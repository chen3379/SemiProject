...
<%
    ...
    if(genre.equals("all")) {
        ...
        else if(sortBy.equals("release_day")) list = dao.getReleaseDayList(startNum, perPage);
        else list = dao.getCreateDayList(startNum, perPage);
    } else {
        if(sortBy.equals("rating")) list = dao.getRatingListByGenre(genre, startNum, perPage);
        else if(sortBy.equals("release_day")) list = dao.getReleaseDayListByGenre(genre, startNum, perPage);
        else list = dao.getCreateDayListByGenre(genre, startNum, perPage);
    }
%>
    ...
    <%
            ...
            for(MovieDto dto : list) {
                ...
                if(dbPoster != null && !dbPoster.isEmpty()) {
                    if(dbPoster.startsWith("http")) fullPosterPath = dbPoster;
                    else fullPosterPath = "../save/" + dbPoster;
                }
    %>
        <div class="col">
            <div class="movie-card" onclick="location.href='movieDetail.jsp?movie_idx=<%=dto.getMovieIdx()%>'">
                <div class="poster-wrapper">
                    <img src="<%=fullPosterPath %>" onerror="this.src='../save/no_image.jpg'" alt="<%=dto.getTitle()%>">
                </div>
                <div class="movie-title"><%=dto.getTitle()%></div>
                <div class="movie-info d-flex justify-content-between">
                    <span>
                        <%= (dto.getReleaseDay() != null && dto.getReleaseDay().length() >= 4) ? dto.getReleaseDay().substring(0, 4) : "" %>
                    </span>
                    <span><i class="bi bi-star-fill text-warning"></i> <%= dto.getReadcount() > 0 ? "인기" : "신규" %></span>
                </div>
            </div>
        </div>
    <%
...
<div class="mt-5">
    <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            ...
            <% if (startPage > 1) { %>
            <li class="page-item">
                <button type="button" class="page-link" onclick="loadMovieList(<%=startPage - 1%>)">
            ...
            <% for (int pp = startPage; pp <= endPage; pp++) { %>
                <li class="page-item <%=pp == currentPage ? "active" : ""%>">
                    <button type="button" class="page-link" onclick="loadMovieList(<%=pp%>)"><%=pp%></button>
                </li>
            <% } %>
            
            <% if (endPage < totalPage) { %>
            <li class="page-item">
                <button type="button" class="page-link" onclick="loadMovieList(<%=endPage + 1%>)">
